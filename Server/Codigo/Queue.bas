...
'TODO : Sacar esto de acá que no tiene nada que ver :P

Function NewReadField(ByVal N As Byte, ByVal Text As String, ByVal separaDor As String) As String

' \ Author : maTih.-
' \ Note   : Función readField ... ¿ mejorada ?

Dim tmp()       As String
Dim finalPos    As Byte

tmp = Split(Text, separaDor)
finalPos = (N - 1)

'Analizo el array.
If tmp(UBound(tmp())) <> vbNullString Then
    'Si finalPos > ultimo elemento, nos vamos al ultimo.
    If finalPos > UBound(tmp()) Then finalPos = UBound(tmp())
        NewReadField = tmp(finalPos)
End If
