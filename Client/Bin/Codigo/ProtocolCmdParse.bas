Attribute VB_Name = "Mod_CMD_Protocol"
'Argentum Online
'
    ...
    If Left$(Comando, 1) = "/" Then
        ...
        Select Case Comando
            ...
            Case "/LIMPIAR"
                ...
                If notNullArguments Then
                    TmpArgs = Split(ArgumentosRaw, "@")
                    
                    'Cuantos elementos?
                    '1 solo?
                    If UBound(TmpArgs()) = 0 Then
                       'Limpia su mapa.
                       Call Protocol.WriteCleanWorld(Val(TmpArgs(0)), Val(TmpArgs(0)))
                    ElseIf UBound(TmpArgs()) = 1 Then
                       'Limpia dos mapas.
                       Call Protocol.WriteCleanWorld(Val(TmpArgs(0)), Val(TmpArgs(1)))
                    End If
                Else
                    Call WriteCleanWorld(0, 0)
                End If
