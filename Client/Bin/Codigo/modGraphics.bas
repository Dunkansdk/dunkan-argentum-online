Attribute VB_Name = "Mod_DX8_Graphics"
Option Explicit
...
Public Function Engine_UTOV_Particle(ByVal UserIndex As Integer, _
                                    ...
                                    ByVal Particle_ID As Byte) As Integer

' / Author: Dunkansdk
' / Note: Crea las partículas en los char's ( USERINDEX > VICTIMINDEX u ONLY USERINDEX)

' maTih.-  :  Modifiqué la función, ahora devuelve un boolean si se casteó el hechizo
'             Porque si no , no podiamos diferenciar un hechizo sin particula de uno con.
...
Select Case Particle_ID
    ...
    Case ParticleIndex.eApocalipsis              '< Apoca
        X = Engine_TPtoSPX(charlist(UserIndex).Pos.X)
        Y = Engine_TPtoSPY(charlist(UserIndex).Pos.Y) + 5

        TempIndex = Effect_Curse_Apocalipsis_Begin(X, Y, 1, 300, 179, 200)
                    Effect(TempIndex).BindToChar = VictimIndex
                    Effect(TempIndex).BindSpeed = 8
                    
    Case ParticleIndex.eInmovilizar              '< Inmo!
        X = Engine_TPtoSPX(charlist(UserIndex).Pos.X)
        Y = Engine_TPtoSPY(charlist(UserIndex).Pos.Y)

        TempIndex = Effect_Curse_Inmovilizar_Begin(X, Y, 1, 300, 179, 200)
                    Effect(TempIndex).BindToChar = VictimIndex
                    Effect(TempIndex).BindSpeed = 8
                    
    Case ParticleIndex.eMeditarGRAN              '< Meditar
        X = Engine_TPtoSPX(charlist(UserIndex).Pos.X) + 10
        Y = Engine_TPtoSPY(charlist(UserIndex).Pos.Y) - 15
        
       TempIndex = Effect_Meditation_Begin(X, Y - 20, 1, 500, 1)

    Case ParticleIndex.eMeditarGIGA             '< Meditar
        X = Engine_TPtoSPX(charlist(UserIndex).Pos.X) + 10
        Y = Engine_TPtoSPY(charlist(UserIndex).Pos.Y) - 15
...
Public Sub Engine_LoadMap_Connect()
    ...
    If FileExist(App.path & "\..\Resources\Mapas\Mapa" & CStr(UserMap) & ".map", vbNormal) Then
