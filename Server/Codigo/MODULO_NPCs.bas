...
'***************************************************
    ...
    ' maTih.-   /   Preparo el equipamiento del npc.
    Call mod_DunkanGeneral.Preparar_CharNpc(.Char, tempEqpChar)
    
        If Not toMap Then
            Call WriteCharacterCreate(sndIndex, .Char.body, .Char.Head, .Char.heading, .Char.CharIndex, X, Y, tempEqpChar.WeaponAnim, tempEqpChar.ShieldAnim, 0, 0, tempEqpChar.CascoAnim, .Name, 5, 0)
...
Arma = ObjData(Npclist(NpcIndex).Char.WeaponAnim).WeaponAnim
Escudo = ObjData(Npclist(NpcIndex).Char.WeaponAnim).ShieldAnim
casco = ObjData(Npclist(NpcIndex).Char.WeaponAnim).CascoAnim
    If NpcIndex > 0 Then
        With Npclist(NpcIndex).Char
            .body = body
            .Head = Head
            ...
            Call SendData(SendTarget.ToNPCArea, NpcIndex, PrepareMessageCharacterChange(body, Head, heading, .CharIndex, Arma, Escudo, 0, 0, casco))
        End With
    End If
End Sub
...
'###################################################
    ...
    With Npclist(NpcIndex)
        ...
        .Char.WeaponAnim = val(GetVar(App.Path & "\Dat\NPCs.dat", "NPC" & NpcNumber, "Arma"))
        .Char.ShieldAnim = val(GetVar(App.Path & "\Dat\NPCs.dat", "NPC" & NpcNumber, "Escudo"))
        .Char.CascoAnim = val(GetVar(App.Path & "\Dat\NPCs.dat", "NPC" & NpcNumber, "Casco"))
