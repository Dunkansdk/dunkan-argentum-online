...
On Error GoTo Errhandler
    ...
    'Si es gm lo quito de la lista.
    If EsGM(UserIndex) Then Call mod_DunkanGMs.DesConectarAdmin(UserIndex)
...
'03/12/2009: Budi - Optimización del código
'29/10/2010: TonchitoZ - Agregado sistema de Cuentas para PJs
'***************************************************
Dim N As Integer
Dim tStr As String
Dim i   As Long
...
With UserList(UserIndex)
    ...
    '¿No es válida su clase?
    If Server_Info.Clase(.Clase) = False Then
       Call WriteErrorMsg(UserIndex, "Su clase no está permitida en el servidor.")
       Call FlushBuffer(UserIndex)
       Call CloseSocket(UserIndex)
       Exit Sub
    End If
   ...
   'SendData SendTarget.toMap, 1, DunkanSend_UpdateOns()
    ...
    'Mapa del servidor
    If Server_Info.Mapa.map <> 0 Then
    ...
    'Si es gm lo agrega a la lista - maTih.-
    'If EsGM(userIndex) Then Call mod_DunkanGMs.ConectarAdmin(userIndex)
    ...
    'WriteDaoParchamentMessage UserIndex, "Hola salame con papas fritas, el pergamino da asko."
